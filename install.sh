...
echo -e "${BLUE}"
echo "╔════════════════════════════════════════════════════════════╗"
echo "║     DigitalTutor - Мастер установки v1.0                  ║"
...
# Создаём директории
mkdir -p "$INSTALL_DIR"/{postgres,minio,nocodb,backups,logs,master-config,web/student,nginx,n8n-workflows,database}

# Копируем файлы
cp "$SCRIPT_DIR/docker-compose.yml" "$INSTALL_DIR/" 2>/dev/null || echo "docker-compose.yml не найден"
